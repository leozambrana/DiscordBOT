...
for (const file of commandFiles){
    ...
    if('data' in command && 'execute' in command){
        ...
        console.log(`Esse comnado em ${filePath} esta com "data" ou "execute ausentes"`)
    }
}
...
//listener interactions with bot
client.on(Events.InteractionCreate, async interaction => {
    ...
    if(!command){
        console.error('Commmando nao encontrado')
        return
    }
    try {
        ...
        console.error(error)
        await interaction.reply('Houve um erro ao executar o programa')
    }
})